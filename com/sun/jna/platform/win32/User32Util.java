...
public final class User32Util {
    ...
    public static String loadString(String location) throws UnsupportedEncodingException {
        ...
        Memory lpBuffer = new Memory(Native.POINTER_SIZE);
        x = User32.INSTANCE.LoadString(target, index, lpBuffer, 0);
        if (0 == x) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
        if (W32APITypeMapper.DEFAULT == W32APITypeMapper.UNICODE) {
            return new String(((Pointer)lpBuffer).getPointer(0L).getCharArray(0L, x));
        }
        return new String(((Pointer)lpBuffer).getPointer(0L).getByteArray(0L, x), Native.getDefaultStringEncoding());
    }
