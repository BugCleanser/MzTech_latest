...
extends CtBehavior {
    ...
    final String getStringRep() {
        if (this.cachedStringRep == null) {
            this.cachedStringRep = String.valueOf(this.methodInfo.getName()) + Descriptor.getParamDescriptor(this.methodInfo.getDescriptor());
        }
    ...
    public String getLongName() {
        return String.valueOf(this.getDeclaringClass().getName()) + "." + this.getName() + Descriptor.toString(this.getSignature());
    }
    ...
    public void setArgName(int offset, String name) {
        ...
        lva.setVariableName((Modifier.isStatic(this.getModifiers()) ? 0 : 1) + offset, name);
    }

    public String[] getArgNames(int offset) throws NotFoundException {
        CodeAttribute ca = this.methodInfo.getCodeAttribute();
        LocalVariableAttribute lva = (LocalVariableAttribute)ca.getAttribute("LocalVariableTable");
        String[] rss = new String[this.getParameterTypes().length];
        int pos = Modifier.isStatic(this.getModifiers()) ? 0 : 1;
        int i = 0;
        while (i < rss.length) {
            rss[i] = lva.variableName(i + pos + offset);
            ++i;
        }
        return rss;
    }
