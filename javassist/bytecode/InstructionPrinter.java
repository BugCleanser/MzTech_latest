...
implements Opcode {
    ...
    public void print(CtMethod method) {
        ...
        while (iterator.hasNext()) {
            ...
            this.stream.println(String.valueOf(pos) + ": " + InstructionPrinter.instructionString(iterator, pos, pool));
        }
    }
    ...
    public static String instructionString(CodeIterator iter, int pos, ConstPool pool) {
        ...
        switch (opcode) {
            case 16: {
                return String.valueOf(opstring) + " " + iter.byteAt(pos + 1);
            }
            case 17: {
                return String.valueOf(opstring) + " " + iter.s16bitAt(pos + 1);
            }
            case 18: {
                return String.valueOf(opstring) + " " + InstructionPrinter.ldc(pool, iter.byteAt(pos + 1));
            }
            case 19: 
            case 20: {
                return String.valueOf(opstring) + " " + InstructionPrinter.ldc(pool, iter.u16bitAt(pos + 1));
            }
            case 21: 
            case 22: 
            case 23: 
            ...
            case 198: 
            case 199: {
                return String.valueOf(opstring) + " " + (iter.s16bitAt(pos + 1) + pos);
            }
            case 132: {
                return String.valueOf(opstring) + " " + iter.byteAt(pos + 1) + ", " + iter.signedByteAt(pos + 2);
            }
            case 167: 
            case 168: {
                return String.valueOf(opstring) + " " + (iter.s16bitAt(pos + 1) + pos);
            ...
            case 180: 
            case 181: {
                return String.valueOf(opstring) + " " + InstructionPrinter.fieldInfo(pool, iter.u16bitAt(pos + 1));
            }
            case 182: 
            case 183: 
            case 184: {
                return String.valueOf(opstring) + " " + InstructionPrinter.methodInfo(pool, iter.u16bitAt(pos + 1));
            }
            case 185: {
                return String.valueOf(opstring) + " " + InstructionPrinter.interfaceMethodInfo(pool, iter.u16bitAt(pos + 1));
            }
            case 186: {
                return String.valueOf(opstring) + " " + iter.u16bitAt(pos + 1);
            }
            case 187: {
                return String.valueOf(opstring) + " " + InstructionPrinter.classInfo(pool, iter.u16bitAt(pos + 1));
            }
            case 188: {
                return String.valueOf(opstring) + " " + InstructionPrinter.arrayInfo(iter.byteAt(pos + 1));
            }
            case 189: 
            case 192: {
                return String.valueOf(opstring) + " " + InstructionPrinter.classInfo(pool, iter.u16bitAt(pos + 1));
            ...
            case 200: 
            case 201: {
                return String.valueOf(opstring) + " " + (iter.s32bitAt(pos + 1) + pos);
            }
        }
        return opstring;
    }
    ...
    private static String wide(CodeIterator iter, int pos) {
        ...
        switch (opcode) {
            ...
            case 132: 
            case 169: {
                return String.valueOf(opcodes[opcode]) + " " + index;
            }
        }
