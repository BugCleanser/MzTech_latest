...
public class DefineClassHelper {
    ...
    public static Class<?> toClass(Class<?> neighbor, byte[] bcode) throws CannotCompileException {
        return MzTech.unsafe.defineClass(null, bcode, 0, bcode.length, neighbor.getClassLoader(), null);
    }

    public static Class<?> toClass(MethodHandles.Lookup lookup, byte[] bcode) throws CannotCompileException {
        try {
            return MzTech.unsafe.defineClass(null, bcode, 0, bcode.length, lookup.lookupClass().getClassLoader(), null);
        }
        catch (Throwable e) {
    ...
    static Class<?> toPublicClass(String className, byte[] bcode) throws CannotCompileException {
        try {
            return MzTech.unsafe.defineClass(className, bcode, 0, bcode.length, ClassLoader.getSystemClassLoader(), null);
        }
        catch (Throwable t) {
    ...
    extends Helper {
        ...
        private final ReferencedUnsafe getReferencedUnsafe() {
            List<Method> defineClassMethod;
            SecurityActions.TheUnsafe usf;
            block6: {
                try {
                ...
                try {
                    usf = SecurityActions.getSunMiscUnsafeAnonymously();
                    defineClassMethod = usf.methods.get("defineClass");
                    if (defineClassMethod != null) break block6;
                    return null;
                }
                catch (Throwable e) {
                    throw new RuntimeException("cannot initialize", e);
    ...
    extends Helper {
        ...
        /* synthetic */ JavaOther(JavaOther javaOther, JavaOther javaOther2) {
            this();
        }
    }
}
