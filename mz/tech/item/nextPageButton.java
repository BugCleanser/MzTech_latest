...
extends GlassPane {
    ...
    public void set(GuideView view, int slot) {
        super.set(view, slot);
        this.view = view;
        view.getInventory().setItem(slot, new ItemStackBuilder(view.getInventory().getItem(slot)).setLocName("\u00a7a\u4e0b\u4e00\u9875").setCount(view.getMaxPage() - view.page + 1).build());
    }

    @Override
    public void onClick(HumanEntity humanEntity) {
        if (this.view.page < this.view.getMaxPage()) {
