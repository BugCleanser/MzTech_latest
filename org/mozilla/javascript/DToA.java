...
class DToA {
    ...
    static int JS_dtoa(double d, int mode, boolean biasUp, int ndigits, boolean[] sign, StringBuilder buf) {
        ...
        S2 = BigInteger.valueOf(1L);
        if (s5 > 0) {
            S2 = DToA.pow5mult(S2, s5);
        }
        ...
        if (ilim <= 0 && mode > 2) {
            if (ilim < 0 || (i = b.compareTo(S2 = S2.multiply(BigInteger.valueOf(5L)))) < 0 || i == 0 && !biasUp) {
                buf.setLength(0);
                buf.append('0');
                return 1;
            }
        ...
        if (leftright) {
            ...
            i = 1;
            while (true) {
                BigInteger[] divResult = b.divideAndRemainder(S2);
                b = divResult[1];
                dig = (char)(divResult[0].intValue() + 48);
                j = b.compareTo(mlo);
                BigInteger delta = S2.subtract(mhi);
                int n = j1 = delta.signum() <= 0 ? 1 : b.compareTo(delta);
                ...
                if (j < 0 || j == 0 && mode == 0 && (DToA.word1(d) & 1) == 0) {
                    if (j1 > 0 && ((j1 = (b = b.shiftLeft(1)).compareTo(S2)) > 0 || j1 == 0 && ((dig & '\u0001') == 1 || biasUp))) {
                        char c = dig;
                        dig = (char)(dig + '\u0001');
                        if (c == '9') {
                            buf.append('9');
