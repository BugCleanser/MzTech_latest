...
public class Parser {
    ...
    private void parseFunctionParams(FunctionNode fnNode) throws IOException {
        ...
        do {
            ...
            if ((tt = this.peekToken()) == 84 || tt == 86) {
                ...
                this.defineSymbol(88, pname, false);
    ...
    private FunctionNode function(int type, boolean isGenerator) throws IOException {
        ...
        int n = lpPos = this.currentToken == 88 ? this.ts.tokenBeg : -1;
    ...
    private void arrowFunctionParams(FunctionNode fnNode, AstNode params, Map<String, Node> destructuring, Set<String> paramNames) {
        ...
        } else if (params instanceof InfixExpression && params.getType() == 90) {
    ...
    private AstNode statement() throws IOException {
        ...
        try {
            ...
            if (pn != null) {
                ...
                if ((ntt = this.peekToken()) == 162 && pn.getLineno() == this.scannedComments.get(this.scannedComments.size() - 1).getLineno()) {
    ...
    private AstNode statementHelper() throws IOException {
        ...
        switch (tt) {
            ...
            case 154: {
                ...
                if (pn instanceof VariableDeclaration && this.peekToken() == 83) break;
                return pn;
            }
            case 4: 
            case 73: {
        ...
        block18: {
            ...
            try {
                ...
                block9: while (true) {
                    ...
                    while ((tt = this.peekToken()) != 87 && tt != 116 && tt != 117 && tt != 0) {
                        if (tt == 162) {
    ...
    private AstNode letStatement() throws IOException {
        ...
        int lineno = this.ts.lineno;
        int pos = this.ts.tokenBeg;
        AstNode pn = this.peekToken() == 88 ? this.let(true, pos) : this.variables(154, pos, true);
        pn.setLineno(lineno);
        return pn;
    }
    ...
    void defineSymbol(int declType, String name, boolean ignoreNotInBlock) {
        ...
        if (symbol != null && (symDeclType == 155 || declType == 155 || definingScope == this.currentScope && symDeclType == 154)) {
            this.addError(symDeclType == 155 ? "msg.const.redecl" : (symDeclType == 154 ? "msg.let.redecl" : (symDeclType == 123 ? "msg.var.redecl" : (symDeclType == 110 ? "msg.fn.redecl" : "msg.parm.redecl"))), name);
            return;
        }
        switch (declType) {
            case 154: {
        ...
        while (true) {
            switch (tt) {
                case 146: {
                    ...
                    this.mustMatchToken(86, "msg.syntax", true);
                    int beg = this.ts.tokenBeg;
                    AstNode expr = this.peekToken() == 87 ? new EmptyExpression(beg, this.ts.tokenEnd - beg) : this.expr();
        ...
        block7: while (true) {
            ...
            switch (tt) {
                ...
                case 147: {
                    ...
                    if (this.mustMatchToken(89, "msg.no.paren", true)) {
                        rp = this.ts.tokenBeg;
                        end = this.ts.tokenEnd;
                    }
                ...
                case 88: {
                    if (!allowCallSyntax) break block7;
                    int lineno = this.ts.lineno;
                    this.consumeToken();
    ...
    private AstNode parenExpr() throws IOException {
        ...
        try {
            ...
            AstNode astNode = e = this.peekToken() == 89 ? new EmptyExpression(begin) : this.expr();
            if (this.peekToken() == 120) {
    ...
    private ObjectLiteral objectLiteral() throws IOException {
        ...
        while (true) {
            ...
            this.getAndResetJsDoc();
            if (!this.matchToken(90, true)) break;
            afterComma = this.ts.tokenEnd;
        }
